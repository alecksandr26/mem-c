...
static void mem_dbg_fetch_memstats_from_page(const int8_t *page_ptr, MemStats_T *stats, int verbose)
{
  ...
  while (ptr < page.available) {
    ...
    if (freed) {
      ...
      stats->nnonfreedchks++;
      stats->usedmem_byu -= 2 * sizeof(uint64_t);
      stats->nonusedmem_byu += 2 * sizeof(uint64_t);
    }
...
{
  uint8_t *ptr = (uint8_t *) addr - 2 * sizeof(uint64_t);
  uint8_t *pageptr = CHKPTR_PAGEPTR(ptr);
...
/* 			if (chk.size < CHK_MIN_CHUNK_SIZE || chk.capacity < CHK_MIN_CHUNK_SIZE - 16) */
